...
class LifecycleRuleFilter(object):
    ...
    def to_sql_query(self, formated_time=None, **kwargs):
        ...
        _slo_cond = ' LEFT JOIN properties pr ON al.alias=pr.alias AND ' \
        ...
        if self.lesser is not None:
            _lesser = f' AND ' \
                f'((ct.size <{self.lesser} AND value IS NULL) OR ' \
                f'value < {self.lesser})'
        ...
        if self.greater is not None:
            ...
            _greater = f' AND ' \
                f'((ct.size >{self.greater} AND value IS NULL) OR '\
                f'value > {self.greater}) '


        if len(self.tags) > 0:
            _base_tag = ' INNER JOIN properties pr2 ON al.alias = pr2.alias '\
        ...
        if formated_time is not None:
            _time_cond = f' AND (al.mtime + {formated_time}) < (CAST '\
...
class Transition(LifecycleAction):
    ...
    # TODO adapt to supported policies: ordered from highest to lowest
    POLICY_ORDER = ('GLACIER', 'ARCHIVE', 'INTELLIGENT-TIERING',
